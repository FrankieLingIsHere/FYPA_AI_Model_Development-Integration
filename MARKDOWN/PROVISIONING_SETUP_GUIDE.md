...
- `CASM_STATE_DIR` (recommended local default: `C:\CASM_System\CASM_LocalState`)
...
- Direct static path `/static/CASM_LocalInstaller.bat` is blocked.
...
| `RESEND_FROM_EMAIL` (optional) | sender identity (e.g. `CASM Alerts <alerts@your-domain.com>`) | Verified sender/domain in Resend | Any time |
...
- If CASM is already installed, clicking that same BAT now performs a lightweight source refresh from the installer snapshot URL and then launches the app.
...
If local source files are refreshed/reinstalled, keep `CASM_STATE_DIR` on a stable path so `machine_id` and provisioning state persist across updates.
