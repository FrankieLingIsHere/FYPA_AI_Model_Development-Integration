document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('../Trim1.csv')
        ...
        .then(csvText => {
            ...
            console.log("CSV data loaded and parsed with d3-dsv:", data);
            incidentData = data;
            analyzeBtn.disabled = false;
            analyzeBtn.textContent = 'Analyze Scene';
            console.log(`Incident database ready. ${incidentData.length} records loaded.`);
        })
        .catch(err => {
            console.error("Failed to load or parse incident data:", err);
            alert("Could not load the incident database. Please ensure 'Trim1.csv' is in the correct directory and the server is running.");
            analyzeBtn.textContent = 'DB Load Failed';
        });
    ...
    function findSimilarIncidents(description, count) {
        ...
        const scoredIncidents = incidentData.map(incident => {
    ...
    function buildPrompt(description, contextIncidents) {
        ...
        return `
        You are an expert AI safety inspector. Your task is to analyze a workplace scene description and identify safety issues based on the provided context.
        Your response MUST be a single, valid JSON object, with no other text or formatting.

        First, review the following real-world incident reports to understand the types of hazards that can occur:
        ---
        Here are some similar incidents for context:

${contextText}
        ---

        Now, analyze the new scene provided below. Based on your analysis and the contextual incidents, generate a detailed JSON report with the following exact structure:
        {
          ...
          "suggested_actions": ["A list of corrective actions to mitigate risks and improve safety."],
