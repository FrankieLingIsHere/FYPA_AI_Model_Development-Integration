...
class ReportRoutingStaticTest(unittest.TestCase):
    ...
    def test_synced_local_tag_survives_cloud_mode_repair(self):
        ...
        self.assertIn('localReportIdLooksLocal', reports_js)
    ...
    def test_cloud_heartbeat_and_provisioning_labels_stay_visible(self):
        ...
        self.assertIn('credentials_imply_local_runtime', casm_app)
        self.assertIn('diagnostics_present = bool(diagnostics)', casm_app)
        self.assertIn('record = {**existing, **(record or {})}', casm_app)
        ...
        self.assertIn('heartbeatRefreshPollBusy', settings_js)
        ...
        self.assertIn('remoteHeartbeatMissing', settings_js)
        self.assertIn('Cloud has not received a paired local backend heartbeat yet', settings_js)
