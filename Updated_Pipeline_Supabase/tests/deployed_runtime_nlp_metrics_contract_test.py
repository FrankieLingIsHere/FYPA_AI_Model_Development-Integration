...
NON_BLOCKING = str(os.environ.get("CASM_RUNTIME_NLP_CONTRACT_NON_BLOCKING", "0")).strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
# Section: run the find markers workflow with clear inputs and outputs.
...
def main() -> int:
    try:
        ...
        # Prepare last provider for the next step.
        ...
        if EXPECTED_ROUTING_PROFILE == "cloud":
            # Prepare conflicting for the next step.
